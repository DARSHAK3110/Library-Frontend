...
@Component({
  selector: 'app-change-book-status-modal',
  templateUrl: './change-book-status-modal.component.html',
  styleUrls: ['./change-book-status-modal.component.css']
})
export class ChangeBookStatusModalComponent {
  id: any;
  isError!: boolean;
  floorSelected: any;
  sectionSelected: any;
  ...
  buildForm() {
    ...
    if (this.id > 0) {
      ...
      this.bookService.getBookstatusesatus(this.id).subscribe((res: any) => {
        this.bookService.getBook(res.bookDetailsId).subscribe((res: any) => {
          this.bookHeader = res;
        },
          (error) => {
            console.log(error);
